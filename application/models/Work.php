...
class Work extends CI_Model
{
	...
	{
		$this->db->select_sum('P.uang_makan');
	...
	{
		$this->db->select('P.id_pekerjaan,U.nama as Pemosting,P.nama as Nama_pekerjaan,P.uang_makan,P.upah,P.is_posted,T.img_bukti as bukti,T.totalAmount as total,T.created_at');
		$this->db->from('pekerjaan P');
	...
	public function getRiwayat_k($id = null){
		$this->db->select('P.nama as nama_pekerjaan,P.uang_makan,P.upah,T.work_status');
		$this->db->from('pekerjaan P');
		$this->db->join('trans_getwork T', 'P.id_pekerjaan = T.id_pekerjaan');
		$this->db->join('users U','T.user_getid = U.id_user');
		$this->db->where('U.id_user', $id);
